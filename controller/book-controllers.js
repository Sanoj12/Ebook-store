...
module.exports = {
    addBook: (books, callback) => {
        ...
        db.get().collection('books').insertOne(books).then((data) => {
            ...
            getBookList:(bookId,bookDetails)=>{
                return new Promise(async (resolve, reject) => {
                    await db.get().collection(collection.BOOK_COLLECTS).find({_id:ObjectId(bookId)}).then((books)=>{
                        console.log(response);
                        resolve(response)
                    })
            ...
            getUserList:(userId)=>{
                return new Promise(async (resolve, reject) => {
                let users=db.get().collection(collection.USER_COLLECTS).find({user:ObjectId(userId)}).toArray()
                     resolve(users)
                     console.log(users)
                })
